...
function DocumentEditor({ document, onChange, onSave, onCancel }) {
  ...
  return <section className="document-editor"><div className="editor-header"><div><p className="date-line">{document.isNew ? '新建文档' : '编辑文档'} <span className="live-dot" /> 本地修改</p><h2>{document.title}</h2></div><div className="editor-actions"><button className={`editor-tab ${!preview ? 'active' : ''}`} onClick={() => setPreview(false)}><Edit3 size={14} /> 编辑</button><button className={`editor-tab ${preview ? 'active' : ''}`} onClick={() => setPreview(true)}><Eye size={14} /> 预览</button><button className="icon-btn small" onClick={onCancel} aria-label="取消编辑"><X size={17} /></button></div></div><div className="editor-fields"><div className="editor-title-field"><label htmlFor="document-title">文档标题</label><input id="document-title" value={document.title} onChange={(event) => onChange({ ...document, title: event.target.value })} /></div><div className="editor-title-field"><label htmlFor="document-project">所属项目</label><input id="document-project" value={document.project} onChange={(event) => onChange({ ...document, project: event.target.value })} /></div></div>{preview ? <div className="markdown-preview">{document.content.trim() ? renderMarkdown(document.content) : <span className="preview-empty">暂无内容</span>}</div> : <textarea className="markdown-editor" value={document.content} onChange={(event) => onChange({ ...document, content: event.target.value })} spellCheck="false" aria-label="Markdown 文档内容" /> }<div className="editor-footer"><span>{document.isNew ? '创建后仅保存在本地知识库' : document.path}</span><div><button className="subtle-btn" onClick={onCancel}>取消</button><button className="primary-btn" onClick={() => onSave(document)}><Save size={15} /> {document.isNew ? '创建文档' : '保存修改'}</button></div></div></section>
}

function KnowledgeBase({ documents, importStatus, onImport, onExportBackup, onRestoreBackup, selectedPaths, onToggleSelected, onToggleAll, onDeleteSelected, onCreate, onEdit, editingDocument, onEditChange, onSaveEdit, onCancelEdit }) {
  ...
  return <div className="knowledge-base">
    {editingDocument && <DocumentEditor document={editingDocument} onChange={onEditChange} onSave={onSaveEdit} onCancel={onCancelEdit} />}
    <section className="knowledge-header"><div><p className="date-line">本地知识库 <span className="live-dot" /> 不会同步到远程</p><h1>你的工作记忆，按项目归位。</h1><p className="lede">从 Obsidian 导入 Markdown 文件夹后，工作台会保留本地副本，并按照项目和文档标题整理。</p></div><div className="knowledge-actions"><button className="primary-btn" onClick={onCreate} disabled={Boolean(editingDocument)}><Plus size={17} /> 新建文档</button><div className="knowledge-count"><strong>{documents.length}</strong><span>篇本地文档</span></div></div></section>
